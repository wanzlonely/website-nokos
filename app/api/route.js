...
export async function POST(request) {
  ...
  if (endpoint === 'history') {
    ...
    const data = await Promise.all(
      keys.map(async k => {
        ...
        const itemType = k.startsWith('order:')
          ? 'order'
          : 'deposit';
        ...
        if (
          ...
          try {
            const r = await fetch(
              `${BASE}/v2/deposit/get_status?deposit_id=${id}`,
              {
                headers: {
                  'x-apikey': keyAPI,
                  accept: 'application/json'
          ...
          try {
            const r = await fetch(
              `${BASE}/v1/orders/get_status?order_id=${id}`,
              {
                headers: {
                  'x-apikey': keyAPI,
                  accept: 'application/json'
            ...
            if (
              resData.success &&
              resData.data?.otp_code
            ) {
              await redis.hset(k, {
                status: 'completed',
                otp_code: resData.data.otp_code,
                otp_msg: resData.data.otp_msg
              });
  ...
  if (endpoint === 'deposit_create') {
    ...
    const url =
      `${BASE}${ENDPOINTS.deposit_create}` +
      `?amount=${payload.amount}&payment_id=qris`;
    ...
    if (data.success && data.data) {
      ...
      const diterimaAmt = Number(
        data.data.diterima || payload.amount
      );
    ...
    if (
      ...
      if (
        ...
        const newBalance = await addBalance(
          user.id,
          Number(dep.diterima || dep.amount)
        );
        ...
        return NextResponse.json({
          ...
          msg: 'Deposit berhasil ditambahkan'
        });
      } else if (
        dep &&
        (
  ...
  if (endpoint === 'order_create') {
    ...
    const countryRes = await fetch(
      `${BASE}/v2/countries?service_id=${payload.service_id}`,
      {
        headers: {
          'x-apikey': key,
          accept: 'application/json'
    ...
    const targetCountry =
      countryData.data?.find(
        c =>
          String(c.number_id) ===
          String(payload.number_id)
      ) ||
      countryData.data?.[0];

    const provider =
      targetCountry?.pricelist?.find(
        p =>
          String(p.provider_id) ===
          String(payload.provider_id)
      ) ||
      targetCountry?.pricelist?.[0];
    ...
    const r = await fetch(
      `${BASE}${ENDPOINTS.order_create}?${params}`,
      {
        headers: {
          'x-apikey': key,
          accept: 'application/json'
    ...
    if (data.success && data.data) {
      ...
      data.msg =
        data.message ||
        data.data ||
        'Stock penyedia habis, tunggu beberapa saat.';
    }
  ...
  if (endpoint === 'order_cancel') {
    ...
    const url =
      `${BASE}/v1/orders/set_status` +
      `?order_id=${payload.order_id}&status=cancel`;
